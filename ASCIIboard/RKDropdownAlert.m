...
static float ANIMATION_TIME = .7; //time it takes for the animation to complete in seconds
static int X_BUFFER = 10; //buffer distance on each side for the text
static int Y_BUFFER = 0; //buffer distance on top/bottom for the text
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [titleLabel setFont:[UIFont fontWithName:@"HelveticaNeue-Medium" size:FONT_SIZE]];
...
-(void)title:(NSString*)title message:(NSString*)message backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor time:(NSInteger)seconds
{
    ...
    if (message) {
        ...
        CGRect frame = titleLabel.frame;
        frame.size.height = HEIGHT-2*Y_BUFFER;
        frame.origin.y = Y_BUFFER;
        titleLabel.frame = frame;
    }
